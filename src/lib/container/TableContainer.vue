<template>
  ...
  >
    ...
    <div slot="footer">
      ...
      <div v-else-if="dataSource" class="pull-right">
    ...
    methods: {
      ...
      loadData(params) {
        if (!this.httpInstance) {
          console.warn(
            '[TableContainer] warning: 请设置 "httpInstance" 属性！'
          );
          return false;
        }
        this.loading = true;
          ...
          .get(this.url, {
            params: clearObj({
              page:
                params && params.page
                  ? params.page - 1
                  : this.pageable && this.$route.query.page
      ...
      computedTableHeight() {
        this.$nextTick(() => {
          if (
            this.$refs['baseRef'] &&
            this.$refs['baseRef'].$refs['contentRef'] &&
            this.$refs['baseRef'].$refs['contentRef'].$el
          ) {
            this.tableHeight =
              this.$refs['baseRef'].$refs['contentRef'].$el.clientHeight - 75;
